      ...
      class Zmq < Base
        ...
        def create_socket
          ...
          begin
            ...
            logger.debug { "Establishing connection: #{server_uri}" }
            zmq_error_check(socket.connect(server_uri), :socket_connect)
            logger.debug { "Connection established to #{server_uri}" }
        ...
        def send_request_with_timeout(timeout, attempt = 0)
          ...
          logger.debug { "Sending Request (attempt #{attempt}, #{socket})" }
          zmq_error_check(socket.send_string(@request_data), :socket_send_string)
          logger.debug { "Waiting #{timeout} seconds for response (attempt #{attempt}, #{socket})" }

          if poller.poll(timeout * 1000) == 1
            zmq_error_check(socket.recv_string(@response_data = ""), :socket_recv_string)
            logger.debug { "Response received (attempt #{attempt}, #{socket})" }
          else
            logger.debug { "Timed out waiting for response (attempt #{attempt}, #{socket})" }
