    ...
    module Connectors
      module Common
        ...
        def complete
          ...
          logger.debug { 'Response proceessing complete' }
          @complete_cb.call(self) unless @complete_cb.nil?
        rescue => e
          logger.error { 'Complete callback error encountered' }
          log_exception(e)
          raise
        end

        def data_callback(data)
          logger.debug { 'Using data_callback' }
        ...
        def fail(code, message)
          ...
          logger.debug { "Server failed request (invoking on_failure): #{@error.inspect}" }

          @failure_cb.call(@error) unless @failure_cb.nil?
        rescue => e
          logger.error { "Failure callback error encountered" }
          log_exception(e)
          raise
        ensure
          complete
        ...
        def parse_response
          ...
          logger.debug { "Parsing response from server (connection closed)" }
        ...
        def succeed(response)
          logger.debug { "Server succeeded request (invoking on_success)" }
          @success_cb.call(response) unless @success_cb.nil?
        rescue => e
          logger.error { "Success callback error encountered" }
          log_exception(e)
        ...
        def verify_callbacks
          unless any_callbacks?
            logger.debug { "No callbacks set, using data_callback" }
