  ...
  module Rpc
    class ServiceDirectory
      ...
      def start
        unless running?
          init_socket
          signed_logger.info { "listening to udp://#{self.class.address}:#{self.class.port}" }
      ...
      def stop
        signed_logger.info { "Stopping directory" }
      ...
      def add_or_update_listing(uuid, server)
        ...
        signed_logger.debug { "#{action} server: #{server.inspect}" }
      end
      ...
      def process_beacon(beacon)
        ...
        if server && uuid
          ...
          signed_logger.info { "Ignoring incomplete beacon: #{beacon.inspect}" }
        end
      end
      ...
      def remove_expired_listings
        signed_logger.debug { "Removing expired listings" }
      ...
      def remove_listing(uuid)
        ...
        signed_logger.debug { "Removing listing: #{listing.inspect}" }
      ...
      def run
        ...
        signed_logger.debug { "ERROR: (#{e.class}) #{e.message}\n#{e.backtrace.join("\n")}" }
        retry
      end
